...
func abrirShortcutDesdeLink(_ urlString: String, storeManager: StoreManager, freeLinks: [String] = ["https://www.icloud.com/shortcuts/ad8ac70847464936990ee2667e68df91"]) {
    ...
    if !isSubscribed && !shortcutLinkFree.contains(urlString) {
        ...
        UserDefaults.standard.set(
            NSLocalizedString("Requires Annual Subscription", comment: ""),
            forKey: "lastMessage"
        )
        return
    }
...
// ================================================================
// MARK: - MainView (Scroll vertical con franjas fijas)
// ================================================================
struct MainView: View {
    ...
    @State private var lastMessage: String = NSLocalizedString("Swipe Down", comment: "")
    ...
    private let rowTitles = [
        ...
        NSLocalizedString("FRANJASEASON", comment: "")
    ]
    ...
    private let IntegratedPlaylistsImages = [
        "sweet","rock","wakeup","yourlikes","piano","allout2010s","magnetismo","hbo","heaven","hymns"
    ]

    private let IntegratedPlaylistsName: [String] = [
        "Sweet Nothing Beat","Rock N Roll Wake Up","Wake up Happy","My liked songs - Mis me gusta","Gentle Piano","All out 2010s","Magnetismo","HBO theme songs","The other side of heaven","Hymns on violin and piano"
    ]

    private let IntegratedPlaylistsDescription = [
        NSLocalizedString("DESC_Sweet Nothing Beat", comment: ""),
        NSLocalizedString("DESC_Rock N Roll Wake Up", comment: ""),
        NSLocalizedString("DESC_Wake up Happy", comment: ""),
        NSLocalizedString("DESC_My liked songs", comment: ""),
        NSLocalizedString("DESC_Gentle Piano", comment: ""),
        NSLocalizedString("DESC_All out 2010s", comment: ""),
        NSLocalizedString("DESC_Magnetismo", comment: ""),
        NSLocalizedString("DESC_HBO theme songs", comment: ""),
        NSLocalizedString("DESC_The other side of heaven", comment: ""),
        NSLocalizedString("DESC_Hymns on violin and piano", comment: "")
    ]

    private let CuradasPlaylistsImages = [
        "Beatles without the Beatles","op de fiets","ROCK is still amazing","Love Songs"
    ]

    private let CuradasPlaylistsName: [String] = [
        "Beatles without the Beatles","op de fiets","ROCK is still amazing","Love Songs"
    ]

    private let CuradasPlaylistsDescription = [
        NSLocalizedString("Beatles without the Beatles", comment: ""),
        NSLocalizedString("op de fiets", comment: ""),
        NSLocalizedString("ROCK is still amazing", comment: ""),
        NSLocalizedString("Love Songs", comment: "")
    ]

    private let shortcutLinksCuradas = [
        "https://www.icloud.com/shortcuts/8d115b8c185145bc8689db47cd4c1533",
        "https://www.icloud.com/shortcuts/10d30a121bb440dfbd3a1a1b045cf460",
        "https://www.icloud.com/shortcuts/43b46bba03434876a06da7e666901c39",
        "https://www.icloud.com/shortcuts/1410cc93f40c41d7bdf97193c455bb04"
    ]
    ...
    private let SingleSongsImagesEN = ["marianastrench","gymnopedie","goodmorning"]
    private let SingleSongsImagesES = ["megustas","otrapiel","noaguanto","comoeres"]
    ...
    private let TextoMenuINGLES = [
        NSLocalizedString("In Mariana's Trench – ABR", comment: ""),
        NSLocalizedString("Gymnopédie No 1 – E. Satie", comment: ""),
        NSLocalizedString("Good Morning – Kanye West", comment: "")
    ]
    ...
    private let TextoMenuESPANOL = [
        NSLocalizedString("Me Gustas Tanto - Miranda", comment: ""),
        NSLocalizedString("Otra Piel - Gustavo Cerati", comment: ""),
        NSLocalizedString("No aguanto - Conociendo Rusia", comment: ""),
        NSLocalizedString("Asi como Eres - Germán Barceló", comment: "")
    ]

    init(activeTab: Binding<Int>, isDarkMode: Binding<Bool>, activeIndex: Binding<Int>) {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack(alignment: .top) {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        VStack(spacing: 0) {
                            HorizontalPlaylistsRowView(
                                H: H,
                                playlists: CuradasPlaylistsImages.indices.map { i in
                                    PlaylistItem(
                                        image: CuradasPlaylistsImages[i],
                                        title: CuradasPlaylistsName[i],
                                        description: CuradasPlaylistsDescription[i],
                                        link: shortcutLinksCuradas[i]
                                    )
                                },
                                title: rowTitles[1],
                                isActive: true,
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            Button(action: {
                                abrirShortcutDesdeLink(shortcutLinkFree[0], storeManager: storeManager)
                            }) {
                                
                                HStack(spacing: 8) {
                                    Text(rowTitles[2])
                                        .font(.custom("AllerDisplay", size: 18))
                                        .foregroundColor(Color("ColorNaranjaIntenso"))
                            ...
                            HStack(alignment: .top, spacing: 12) {
                                ...
                                Text(NSLocalizedString("MinitutorialTEXT", comment: ""))
                        ...
                        VStack(spacing: 0) {
                            VStack(alignment: .leading, spacing: 6) {
                                Text(rowTitles[4])
                                    .font(.custom("AllerDisplay", size: 20))
                                    .foregroundColor(Color("ColorNaranjaIntenso"))
                                Text(NSLocalizedString("PRONTOTEXT", comment: ""))
                                    .foregroundColor(Color("SubtitleColor"))
                                    .font(.system(size: 14))
                            }
                            .frame(maxWidth: .infinity, alignment: .leading)
                    ...
                    .padding(.bottom, 80) // ⚠️ ajusta según altura de la barra inferior + espacio extra
                ...
                // 🔹 Overlay temporal del tip con bounce
                if showTip {
                    VStack {
                        Spacer()
                        HStack(spacing: 8) {
...
// ================================================================
// MARK: - PlaylistItem y HorizontalPlaylistsRowView (FUERA de MainView)
// ================================================================
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            Text(playlist.title)
                ...
                .foregroundColor(Color("ColorNaranja"))
                .lineLimit(1)
                .padding(.top, (playlist.description != nil && !playlist.description!.isEmpty) ? 0 : -2)
        ...
        .frame(width: 140, height: 210, alignment: .topLeading)
    }
}
...
struct OnboardingView: View {
    ...
    var body: some View {
        VStack {
                    ...
                    if !isSubscribed {
                        VStack(spacing: 16) {
                            ...
                            OnboardingPageView(
                                ...
                                description: NSLocalizedString("payementTEXT", comment: "Lifetime Premium description shown in onboarding"),
                                lines: [
                                    (NSLocalizedString("All playlists and features", comment: ""), nil),
                                    (NSLocalizedString("idrequiredTEXT", comment: ""), nil),
            ...
            // ---------- BODY: donde antes tenías `Spacer()` + `continueButton` + restoreButton ----------
        ...
        .onAppear {
            ...
            Task {
                ...
                print("🟢 Subscription status on appear:", isSubscribed)
            }
        }

        .onReceive(store.$products) { _ in
            Task {
                isSubscribed = await store.checkActiveSubscription()
            }
        }
    ...
    // ---------- PREMIUM OVERLAY: ahora contiene los botones de compra rectangulares y los links ----------
    private var premiumOverlay: some View {
        VStack {
            //Spacer()
            VStack(spacing: 12) {
                if !isSubscribed {
                    let lifetimeProduct = store.products.first(where: { $0.id == store.lifetimeProductID })
                    let annualProduct = store.products.first(where: { $0.id == store.annualProductID })
                    
                    // Botones de compra principales (rectangulares, grandes, con degradado)
                    VStack(spacing: 12) {
                            ...
                            Button {
                                ...
                                HStack {
                                    if store.isLoading {
                                        ...
                                        Text(String(format: NSLocalizedString("Lifetime", comment: ""), lifetime.displayPrice))
                                            .font(.system(.headline, design: .default).weight(.bold))
                                    }
                                }
                                .frame(maxWidth: .infinity)
                                ...
                                .foregroundColor(.white)
                                .cornerRadius(12)
                                .shadow(color: Color.purple.opacity(0.28), radius: 6, x: 0, y: 3)
                            }
                            ...
                            Button {
                                Task {
                                    ...
                                    do {
                                        ...
                                        restoreMessage = NSLocalizedString("Purchase failed. Please try again later.", comment: "")
                                        print("❌ Annual subscription failed: \(error)")
                                    }
                                    store.isLoading = false
                                }
                            } label: {
                                HStack {
                                    if store.isLoading {
                                        ...
                                        Text(String(format: NSLocalizedString("OneYear", comment: ""), annual.displayPrice))
                                            .font(.system(.headline, design: .default).weight(.bold))
                                    }
                                }
                                .frame(maxWidth: .infinity)
                                ...
                                .foregroundColor(.white)
                                .cornerRadius(12)
                                .shadow(color: Color.cyan.opacity(0.28), radius: 6, x: 0, y: 3)
                            }
                            .disabled(store.isLoading)
                        }
                    }
                ...
                Button(action: {
                    ...
                    Text(NSLocalizedString("Continue with Free Version", comment: ""))
                        .font(.system(.headline, design: .default).weight(.bold))
                        .frame(maxWidth: .infinity)
                        .frame(height: 56)
                ...
                // Links fila: Terms of use, Restore purchase, Privacy policy (azul)
                HStack(spacing: 20) {
                    // Terms of Use
                    Button(action: {
                        if let url = URL(string: "https://www.myiosapps.org/terms.html") {
                            abrirLinkMenu(url)
                        }
                    }) {
                        Text(NSLocalizedString("Terms of use", comment: ""))
                            .font(.system(size: 14))
                    }
                    ...
                    // Privacy Policy
                    Button(action: {
                        if let url = URL(string: "https://www.myiosapps.org/privacy.html") {
                            abrirLinkMenu(url)
                        }
                    }) {
...
// --- restorePurchase (helper async, reutilizable) ---
func restorePurchase() async -> (found: Bool, message: String?) {
    do {
        ...
        for try await result in Transaction.currentEntitlements {
            if case .verified(let transaction) = result {
                foundPurchase = true
                let msg = NSLocalizedString("Your purchases have been successfully restored.", comment: "")
        ...
        let msg = NSLocalizedString("No active purchases were found to restore.", comment: "")
        print("ℹ️ No active purchases found.")
        return (false, msg)

    } catch {
        // Error en la enumeración
        let msg = NSLocalizedString("Failed to restore purchases. Please try again later.", comment: "")
...
struct RootView: View {
    ...
    private func migrateUserDefaults() {
        ...
        if lastVersion != currentVersion {
            ...
            // defaults.set(false, forKey: "hasSeenOnboarding") // Descomenta si quieres mostrar onboarding en cada versión nueva
        }
    }
}
...
struct SpotiActionsApp: App {
    ...
    init() {
        ...
        // Leer flag de onboarding antes de crear el estado de navegación
