...
struct HeaderView: View {
    ...
    let H: CGFloat
    @Binding var isDarkMode: Bool
    var isShortcutMode: Binding<Bool>?
        ...
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    ...
                    .frame(width: 80, alignment: .leading) // fija zona izquierda
                    ...
                    .frame(width: 80, alignment: .trailing) // fija zona derecha
                }
                ...
                VStack(spacing: 8) {
                    ...
                    if let shortcutBinding = isShortcutMode {
                        HStack(spacing: 10) {
                            Text(NSLocalizedString("ModoInstalar", comment: ""))
                                .font(.system(size: 12, weight: .semibold))
                                .foregroundColor(
                                    shortcutBinding.wrappedValue
                                    ? Color("Color1reverse").opacity(0.5)
                                    : Color("ColorHeaderView")
                                )

                            Toggle("", isOn: shortcutBinding)
                                .labelsHidden()
                                .toggleStyle(SwitchToggleStyle(tint: Color("ColorHeaderView")))
                                .scaleEffect(0.85)   // ← reduce tamaño
                                .onChange(of: shortcutBinding.wrappedValue) { value in
                                    lastMessage = value
                            ...
                            Text(NSLocalizedString("ModoPlay", comment: ""))
                                .font(.system(size: 12, weight: .semibold))
                                .foregroundColor(
                                    shortcutBinding.wrappedValue
                                    ? Color("ColorHeaderView")
...
struct MainView: View {
    ...
    private let SingleSongsES: [[String: String]] = [
        ...
        [
            "nameshortcut": "Así Como Eres",
            "name": "Asi como Eres - Germán Barceló",
            "imageasset": "comoeres",
            "description": NSLocalizedString("Asi como Eres - Germán Barceló", comment: ""),
            "url": "https://www.icloud.com/shortcuts/3452c786889e4409a4d0405b0b4d9aed"
        ]
    ]
    ...
    init(activeTab: Binding<Int>, isDarkMode: Binding<Bool>,isShortcutMode: Binding<Bool>, activeIndex: Binding<Int>) {
        self._activeTab = activeTab
        self._isDarkMode = isDarkMode
        self._isShortcutMode = isShortcutMode
            ...
            ZStack(alignment: .top) {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        // --- FRANJA MAS ESCUCHADAS ---
                        VStack(spacing: 0) {

                            let MoreListenedPlaylists: [PlaylistItem] = MoreListened.map { dict in
                                PlaylistItem(
                                    image: dict["imageasset"] ?? "",
                                    title: dict["name"] ?? "",
                        ...
                        VStack(spacing: 0) {
                            HorizontalPlaylistsRowView(
                                H: H,
                                playlists: IntegratedPlaylists.map { dict in
                                    PlaylistItem(
                                        image: dict["imageasset"] ?? "",
                                        title: dict["name"] ?? "",
                                        description: dict["description"] ?? "",
                        ...
                        VStack(spacing: 0) {
                            HorizontalPlaylistsRowView(
                                H: H,
                                playlists: shortcutLinksCuradas.indices.map { i in
                                    let dict = shortcutLinksCuradas[i]
                                    return PlaylistItem(
                                        image: dict["imageasset"] ?? "",
                                        title: dict["name"] ?? "",
                                        description: dict["description"] ?? "",
                        ...
                        VStack(spacing: 0) {
                            ...
                            let englishPlaylists: [PlaylistItem] = SingleSongsEN.map { dict in
                                PlaylistItem(
                                    image: dict["imageasset"] ?? "",
                                    title: dict["name"] ?? "",
                                    description: dict["description"],
                            ...
                            let spanishPlaylists: [PlaylistItem] = SingleSongsES.map { dict in
                                PlaylistItem(
                                    image: dict["imageasset"] ?? "",
                                    title: dict["name"] ?? "",
                                    description: dict["description"],
                        ...
                        // --- FRANJA FRANJAVERYSPECIFIC ---
                        VStack(spacing: 0) {

                            let verySpecificPlaylists: [PlaylistItem] = VerySpecific.map { dict in
                                PlaylistItem(
                                    image: dict["imageasset"] ?? "",
                                    title: dict["name"] ?? "",
                                    description: dict["description"],
                            ...
                            HorizontalPlaylistsRowView(
                                H: H,
                                playlists: verySpecificPlaylists,
    ...
    } // ← AQUÍ TERMINA EL BODY **BIEN CERRADO**
    
    // ================================================================
    // Función local (CORRECTAMENTE UBICADA)
    // ================================================================
    @MainActor
    func abrirShortcutLocal(_ urlString: String) {
        guard let url = URL(string: urlString) else { return }
        Task { @MainActor in
            UIApplication.shared.open(url)
        }
...
} // ← CIERRE FINAL DEL STRUCT (solo 1)

// ================================================================
// MARK: - PlaylistItem y HorizontalPlaylistsRowView (FUERA del struct)
// ================================================================
struct PlaylistItem: Identifiable {
    ...
    init(image: String, title: String, description: String? = nil, data: [String : String]) {
        self.image = image
        self.title = title
...
struct HorizontalPlaylistsRowView: View {
    ...
    let onOpenShortcut: ([String : String]) -> Void
    ...
    init(
        ...
        sectionIndex: Int = 0,
        onOpenShortcut: @escaping ([String : String]) -> Void
    ) {
        self.H = H
...
struct PlaylistCardView: View {
    ...
    var sectionIndex: Int
    var onOpenShortcut: ([String : String]) -> Void
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            Text(playlist.title)
                ...
                .foregroundColor(Color("ColorNaranja"))
                .lineLimit(2)
                .frame(height: 32, alignment: .bottomLeading) // ← CLAVE

            Button(action: {
                onOpenShortcut(playlist.data)
            }) {
