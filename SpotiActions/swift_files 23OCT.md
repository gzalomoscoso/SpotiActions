                    ...
                    HStack(spacing: 12) {
                        ...
                        if showGear {
                            Menu {
                                ...
                                Divider()
                                Button(NSLocalizedString("buy_coffee", comment: "")) {
                                    abrirLinkMenu(URL(string: "https://www.paypal.com/donate/?hosted_button_id=GTVP7RU6CLHJS")!)
                                }
                            } label: {
...
// ================================================================
@MainActor
func abrirShortcutDesdeLink(_ urlString: String, storeManager: StoreManager) {
#if DEBUG
...
print("🔓 Debug Mode: Forzado isSubscribed = true")
    ...
    // Lista de shortcuts free
    let shortcutLinkFree = [
...
struct MainView: View {
    ...
    private let rowTitles = [
        ...
        NSLocalizedString("YOUR LIKED SONGS IN SPOTIFY", comment: "Section title for liked songs"),
    ...
    private let shortcutLinkFree = ["https://www.icloud.com/shortcuts/ccb8562e1e8548b283d84cac64887272"]
...
class NavigationState: ObservableObject {
    enum ViewType {
        case welcome
        case main
        case subscription   // ✅ Agregamos la pantalla de suscripción (pantalla 5)
        ...
        VStack {
                ...
                if store.isLoading {
                    ...
                    TabView(selection: $currentPage) {
                        ...
                        OnboardingPageView(
                            ...
                            commontextLines: [
                                OnboardingLine(text: NSLocalizedString("At a set time (perfect as a wake-up alarm)", comment: ""), url: nil),
                                OnboardingLine(text: NSLocalizedString("At a specific place (so your music plays when you arrive home)", comment: ""), url: nil),
                                OnboardingLine(text: NSLocalizedString("When you hop in your car and connect to CarPlay", comment: ""), url: nil)
                            ]
                        )
                        .tag(0)
                        ...
                        OnboardingPageView(
                            ...
                            commontextLines: [
                                OnboardingLine(text: NSLocalizedString("SpotiActions puts ready-made shortcuts right on the main screen", comment: ""), url: nil),
                                OnboardingLine(text: NSLocalizedString("They can shuffle playlists, play them in order, or play specific songs", comment: ""), url: nil),
                                OnboardingLine(text: NSLocalizedString("Just install and run them once — this gives them permission to work on their own", comment: ""), url: nil),
                                OnboardingLine(text: NSLocalizedString("Install the Shortcuts App if you don’t have it yet", comment: ""), url: URL(string: "https://apps.apple.com/app/atajos/id1462947752"))
                            ]
                        )
                        .tag(1)
                        ...
                        OnboardingPageView(
                            ...
                            commontextLines: [
                                OnboardingLine(text: NSLocalizedString("Sure, you can make one that just opens Spotify — but it won’t run by itself and you’d have to confirm every single time", comment: ""), url: nil),
                                OnboardingLine(text: NSLocalizedString("SpotiActions shortcuts only ask for permission once, after that, they run completely automatically", comment: ""), url: nil)
                            ]
                        )
                        .tag(2)
                        ...
                        OnboardingPageView(
                            ...
                            commontextLines: [
                                OnboardingLine(text: NSLocalizedString("Create an automation that triggers your chosen shortcut", comment: ""), url: nil),
                                OnboardingLine(text: NSLocalizedString("Just follow our easy step-by-step guide and you’ll be set", comment: ""), url: nil),
                                OnboardingLine(text: NSLocalizedString("If you have any trouble setting up, our support team is here to help! Just write to us", comment: ""), url: nil)
                            ]
                        )
                        .tag(3)

                        // --- PANTALLA 5 SOLO SI NO ESTÁ SUSCRITO ---
                        if !isSubscribed {
                            OnboardingPageView(
                                systemName: "creditcard.fill",
                                gradient: [Color.purple, Color.orange],
                                title: NSLocalizedString("Ready to unlock the full experience?", comment: ""),
                                description: NSLocalizedString("The free version only lets you use the playlist of your liked songs", comment: ""),
                                commontextLines: [
                                    OnboardingLine(text: NSLocalizedString("The yearly subscription unlocks access to all playlists", comment: ""), url: nil),
                                    OnboardingLine(text: NSLocalizedString("See how easy it is to automate. Check the tutorials", comment: ""), url: nil),
                                    OnboardingLine(text: NSLocalizedString("You can use url schemes spotiactions://", comment: ""), url: nil)
                                ]
                            )
                            .tag(4)
                            .overlay(
                                VStack {
                                    ...
                                    VStack(spacing: 12) {
                                        ...
                                        } else if let annualProduct = store.products.first, !isSubscribed {
                                            Button(action: {
                                                Task {
                                                    do {
                                                        try await store.purchase(annualProduct)
                                                        UserDefaults.standard.set(true, forKey: "isSubscribed")
                                                        isSubscribed = true
                                                        onFinished?()
                                                    } catch {
                                                ...
                                                Text(String(format: NSLocalizedString("Annual Subscription – %@ / year", comment: ""), annualProduct.displayPrice))
            ...
            .onAppear {
                Task {
                    ...
                    if isSubscribed {
                        onFinished?() // salta el onboarding completo
                    }
                }
            }
            ...
            Button(action: {
                ...
                .frame(maxWidth: .infinity)
                .padding(.vertical, 12) // asegura área táctil
                .contentShape(Rectangle()) // garantiza que el área clickeable sea rectangular
...
// --- OnboardingLine ---
struct OnboardingLine: Identifiable {
    let id = UUID()
    let text: String
    let url: URL?
}
...
struct OnboardingPageView: View {
    ...
    let title: String
    let description: String
    let commontextLines: [OnboardingLine]?
...
// --- Función global para abrir shortcuts ---
...
struct RootView: View {
    ...
    private func handleDeepLink(_ url: URL) {
        if url.scheme == "spotiactions",
           url.host == "tab",
           let tab = Int(url.lastPathComponent) {
            self.activeTab = tab
        }
    }
...
struct SpotiActionsApp: App {
    ...
    @StateObject private var storeManager: StoreManager // inicializamos en init
    ...
    var body: some Scene {
        WindowGroup {
            RootView(isDarkMode: $isDarkMode)
                .environmentObject(navigationState)
                .environmentObject(storeManager) // ✅ Inyectar StoreManager aquí
                .onAppear {
                    let hasSeenOnboarding = UserDefaults.standard.bool(forKey: "hasSeenOnboarding")

                    #if DEBUG
                    // ⏱️ Evita freeze al inicio del modo Debug
...
class StoreManager: ObservableObject {
    ...
    private let productID = "com.spotiactions.premium.anual"
    private let lastPaidVersion = "1.8" // Última versión que fue de pago único
    ...
    func purchase(_ product: Product) async throws {
        ...
        case .userCancelled:
            throw NSError(domain: "Store", code: 1, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("user_cancelled", comment: "")])
        case .pending:
    ...
    private func postVerifyReceipt(to url: URL, receiptData: Data) async throws -> [String: Any] {
        ...
        if let httpResp = response as? HTTPURLResponse, !(200...299).contains(httpResp.statusCode) {
            ...
            throw NSError(domain: "StoreManager",
    ...
    var body: some View {
        VStack(spacing: 20) {
            if store.isLoading {
                ...
                // Intentamos seleccionar explícitamente el product por ID, si no, usamos first
                let annualProduct = store.products.first { $0.id == productID } ?? store.products.first
                ...
                if let annualProduct = annualProduct {
                    Button(action: {
                        ...
                        Text(String(format: NSLocalizedString("continue_with_price", comment: ""), annualProduct.displayPrice))
                            .font(.headline)
                            .foregroundColor(.white)
                            .padding()
                            .frame(maxWidth: .infinity)
